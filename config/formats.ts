...
export const Formats: FormatList = [
	...
	{
		name: "[Gen 8] Random Weather",
		mod: 'gen8',
		ruleset: ['Standard', 'Dynamax Clause'],
		banlist: ['Uber', 'AG', 'Arena Trap', 'Moody', 'Power Construct', 'Shadow Tag', 'Baton Pass', 'Drizzle', 'Drought', 'Sand Stream', 'Snow Warning'],
		onBegin() {
			const rndInt = Math.floor(Math.random() * 4) + 1
			if (rndInt == 1) {
				this.add('-weather', 'move: Sandstorm');
				this.field.weather = 'sandstorm' as ID;
			}
			if (rndInt == 2) {
				this.add('-weather', 'move: Hail');
				this.field.weather = 'hail' as ID;
			}
			if (rndInt == 3) {
				this.add('-weather', 'move: Rain Dance');
				this.field.weather = 'rain' as ID;
			}
			if (rndInt == 4) {
				this.add('-weather', 'move: Sunny Day');
				this.field.weather = 'sun' as ID;
			}
		}
	},
	{
